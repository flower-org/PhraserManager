...
1 - a finite number of records on sectors;  
2 - bit rot.

Since programs operating PhraserDB can run on microcontrollers, writing data/blocks directly to the built-in Flash,
...
extended if similar approaches are applied to evenly distribute the load and regularly copy data, PhraserDB was imagined
...
Note that if block_2 was located immediately after block_1, then after writing its updated version to block_3, block_2 itself becomes obsolete.
Thus, there is a guarantee that the block located immediately "to the right" of block_1 at the time of writing the main update will always
...
At the same time, the last block always shifts exactly one sector "to the right."  
